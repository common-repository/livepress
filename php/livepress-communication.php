...
class LivePress_Communication {
	 ...
	 *
	 * @return string Server HTTP response.
	 * @internal param string $screen_name The twitter username to follow.
	 */
	...
	/**
	 * Updates the Slack team domain, channel and default author associated with the blog.
	 *
	 * @param string $slack_team_domain New Slack team domain.
	 * @param string $slack_channel New Slack channel.
	 * @param string $slack_default_author New Slack defaul author.
	 * @return int HTTP code response.
	 */
	public function set_slack_settings( $slack_team_domain, $slack_channel, $slack_default_author ) {
		$params['slack_team_domain'] = $slack_team_domain;
		$params['slack_channel'] = $slack_channel;
		$params['slack_default_author'] = $slack_default_author;
		$code = $this->request_to_livepress( '/blog/set_slack_settings', 'post', $params );
		return $code;
	}


	/**
	 * Retrieves the current verification code for the Slack channel configured for this blog.
	 *
	 * @param string $shortname New blog shortname.
	 * @return int HTTP code response.
	 */
	public function slack_verification_code( $slack_team_domain, $slack_channel ) {
		$params['slack_team_domain'] = $slack_team_domain;
		$params['slack_channel'] = $slack_channel;
		$code = $this->request_to_livepress( '/blog/slack_verification_code', 'post', $params );
		return $code;
	}
