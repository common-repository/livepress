...
class LivePress_Administration {
	...
	function add_admin_messages() {
		...
		if ( empty( $this->options['api_key'] ) && ! isset( $_POST['submit'] ) && ! function_exists( 'livepress_api_key_missing_warning' ) ) {
			...
			// mkare sure the css is also loaded if this is shown
	 ...
	 * @param string $api_key          API Key, optional.  Only needed when this may be called before API key is saved.
	 * @return bool                    Whether to enable remote posting.
	 */
	public function enable_remote_post( $user_id, $lp_user_password = '', $api_key = '' ) {
		$user          = get_userdata( $user_id );
		$this->options = get_option( self::$options_name );
		$livepress_com = new LivePress_Communication( empty( $api_key ) ? $this->options['api_key'] : $api_key );
	...
	static function plugin_install() {
		...
		if ( false === $livepress || ! array_key_exists( 'update_format', $livepress ) ) {
			$livepress['update_format'] = 'default';
			update_option( self::$options_name, $livepress );
		}
	}
	...
	function add_css_and_js() {
		...
		wp_register_style( 'livepress_facebox', LP_PLUGIN_URL . 'css/facebox.css',array(), LP_PLUGIN_SCRIPT_VERSION );
		wp_register_style( 'livepress_admin', LP_PLUGIN_URL . 'css/admin.css',array(), LP_PLUGIN_SCRIPT_VERSION );
	...
	public static function api_key_validate() {
		...
		if ( $status == 1 ) {
			...
			$options['slack_team_domain'] = $blog->slack_team_domain;
			$options['slack_channel'] = $blog->slack_channel;
	...
	public static function check_oauth_authorization_status() {
		...
		if ( $status->status == 'authorized' ) {
			...
			update_option( self::$options_name, $options );
		} elseif ( $status->status == 'unauthorized' ) {
			$options['post_to_twitter'] = false;
			$options['oauth_authorized_user'] = '';
			update_option( self::$options_name, $options );
		}
