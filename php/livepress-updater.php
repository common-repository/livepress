...
class LivePress_Updater {
	 ...
	 */
	public function remove_related_post_data( $post_id ) {
		global $current_user;
		$login = $current_user->user_login;
		$this->livepress_communication->send_to_livepress_handle_twitter_follow( 'clear', '', $post_id, $login ); // TODO: Handle error
		Collaboration::clear_searches( $post_id, $login ); //clear all term searches for this post/user
	}
	...
	public function has_livepress_enabled() {
		...
		return current_user_can( 'manage_options' )      // the user is admin
			&& 'none' !== $enabled_to     // and not blocked for everyone
			|| 'all' === $enabled_to    // everybody is allowed
			|| 'registered' === $enabled_to
	...
	function add_css_and_js_on_header( $hook = null ) {
		...
		wp_register_style( 'livepress_main_sheets', LP_PLUGIN_URL . 'css/livepress.css', array(), LP_PLUGIN_SCRIPT_VERSION );
		...
		if ( 'profile.php' == $hook && defined( 'WPCOM_IS_VIP_ENV' ) && false !== WPCOM_IS_VIP_ENV ) {
			if ( $this->livepress_config->script_debug() ) {
				wp_enqueue_script( 'livepress-profile', LP_PLUGIN_URL . 'js/livepress-profile.full.js', array( 'jquery' ), LP_PLUGIN_SCRIPT_VERSION );
			} else {
				wp_enqueue_script( 'livepress-profile', LP_PLUGIN_URL . 'js/livepress-profile.min.js', array( 'jquery' ), LP_PLUGIN_SCRIPT_VERSION );
			}
		}
		...
		wp_register_style( 'wordpress_override', LP_PLUGIN_URL . 'css/livepress_wordpress_override.css', array(), LP_PLUGIN_SCRIPT_VERSION );
		wp_enqueue_style( 'wordpress_override' );

		wp_register_style( 'livepress_ui', LP_PLUGIN_URL . 'css/ui.css', array(), LP_PLUGIN_SCRIPT_VERSION, 'screen' );
		...
		if ( is_page() || is_single() || is_home() ) {
			wp_enqueue_script( 'livepress-plugin-loader', LP_PLUGIN_URL . 'js/plugin_loader_release.' . $mode . '.js', array( 'jquery' ), LP_PLUGIN_SCRIPT_VERSION );
			...
			wp_enqueue_style( 'select2', LP_PLUGIN_URL . 'css/select2.css', array(), LP_PLUGIN_SCRIPT_VERSION );
			// Add datetimepicker for Json+LD
			wp_enqueue_style( 'anytime', LP_PLUGIN_URL . 'css/anytime.5.1.2.css', array(), LP_PLUGIN_SCRIPT_VERSION );

			wp_register_style( 'lp_admin', LP_PLUGIN_URL . 'css/post_edit.css', array(), LP_PLUGIN_SCRIPT_VERSION );
			wp_enqueue_style( 'lp_admin' );

			wp_enqueue_style( 'lp_admin_font', LP_PLUGIN_URL . 'fonts/livepress-admin/style.css', array(), LP_PLUGIN_SCRIPT_VERSION );

			wp_enqueue_script( 'lp-admin', LP_PLUGIN_URL . 'js/admin/livepress-admin.' . $mode . '.js', array( 'jquery' ), LP_PLUGIN_SCRIPT_VERSION );
			...
			if ( in_array( $hook, array( 'post.php' ) ) ) {
				wp_enqueue_script( 'dashboard-dyn', LP_PLUGIN_URL . 'js/dashboard-dyn.' . $mode . '.js', array( 'jquery', 'lp-admin' ), LP_PLUGIN_SCRIPT_VERSION );
			}
		}
		...
		if ( 'en' !== $lan[0] ) {
			...
			wp_enqueue_script( 'jquery.timeago.' . $lan[0], LP_PLUGIN_URL . "js/locales/jquery.timeago.$timeago_lan.js", array( 'jquery.timeago' ), LP_PLUGIN_SCRIPT_VERSION );
			$select2_lan = $lan[0];
			switch ( strtolower( get_locale() ) ) {
				case 'pt_br':
					$select2_lan = 'pt-BR';
			...
			wp_enqueue_script( 'select2_locale.' . $lan[0], LP_PLUGIN_URL . "js/locales/select2_locale_$select2_lan.js", array( 'lp-admin' ), LP_PLUGIN_SCRIPT_VERSION );
		}

		$current_theme = str_replace( ' ', '-', strtolower( wp_get_theme()->Name ) );
		if ( file_exists( LP_PLUGIN_PATH . 'css/themes/' . $current_theme . '.css' ) ) {
			wp_register_style( 'livepress_theme_hacks', LP_PLUGIN_URL . 'css/themes/' . $current_theme . '.css', array(), LP_PLUGIN_SCRIPT_VERSION );
	...
	function add_js_config( $hook = null ) {
		...
		$ljsc->new_value( 'ver', LP_PLUGIN_SCRIPT_VERSION );
		...
		// Comments
		if ( isset( $current_post_id ) && 0 !== $current_post_id ) {
			...
			$sl = array();
			$meta = get_post_meta( $current_post_id );
		...
		if ( is_admin() || $is_live ) {
			...
			if ( is_admin() ) {
				...
				if ( $update_author ) {
					...
					$feed_order = apply_filters( 'lp_admin_update_append_location', 'top' );
		...
		/**
		 * Filter Allows you set if you are loading the Facebook JS yourself.
		 * set to true to stop LP loading the Facebook JS file
		 *
		 * @since 1.4.0
		 *
		 * @param bool: is the Facebook_Loader class exits from the FB plugins.
		 *
		 */
		if ( apply_filters( 'lp_facebook_js_loaded', class_exists( 'Facebook_Loader' ) ) ) {
		...
		/*
		 * allows you to change the target for the scroll into view
		 *
		 *  Since 1.3.9
		 *
		 * $CSS_selector string
		 */
		$ljsc->new_value( 'scroll_target_for_post_title',  apply_filters( 'livepress_scroll_target_for_post_title_selector', '.lp-bar' ) );
		/*
		 * allows you to change the target for the scroll into view when load bottom up
		 *
		 *  Since 1.3.17
		 *
		 * $CSS_selector string
		 */
		$ljsc->new_value( 'scroll_target_for_post_bottom',  apply_filters( 'livepress_scroll_target_for_post_bottom_selector', '.lp-credits-after' ) );
		/*
		 * when true the the bottom new update bar has a click action to scroll to new update when false it hides on scroll
		 *
		 *  Since 1.3.17
		 *
		 * $CSS_selector string
		 */
		$ljsc->new_value( 'scroll_target_bottom_click',  apply_filters( 'livepress_scroll_target_bottom_click', 'false' ) );
		/*
		...
		if ( 'bottom' === Pause_Updates::get_display_order( $current_post_id ) ) {
			// translator note ##count## is the number of updates
			$ljsc->new_value( 'view_new_updates', esc_html(
			apply_filters( 'livepress_scroll_target_bottom_text_plural', __( 'Scroll to view ##count## new updates', 'livepress' ) ) ) );
			// translator note ##count## is the number of updates
			$ljsc->new_value( 'view_new_update',esc_html(
			apply_filters( 'livepress_scroll_target_bottom_text', __( 'Scroll to view ##count## new update', 'livepress' ) ) ) );
		} else {
			// translator note ##count## is the number of updates
			$ljsc->new_value( 'view_new_updates', esc_html(
			apply_filters( 'livepress_scroll_target_top_text_plural', __( 'View ##count## new updates', 'livepress' ) ) ) );
			// translator note ##count## is the number of updates
			$ljsc->new_value( 'view_new_update', esc_html(
			apply_filters( 'livepress_scroll_target_top_text', __( 'View ##count## new update', 'livepress' ) ) ) );
		}
		...
		if ( is_admin() ) {
			...
			/**
			 * Filter to allow you reorder the author avatar block.
			 *
			 * @since 1.3.19
			 *
			 * @param string $html with replace target to allow reordering of HTML elements.
			 * @param bool $is_first_post is it a converting the content from the parent post to the first post.
			 */
			$ljsc->new_value( 'author_html_order', apply_filters( 'lp_authors_html_order' , '##id_html## ##avatar## ##author_name##', false ) );
			/**
			 * Filter to allow you change the html for the author ID.
			 *
			 * @since 1.3.19
			 *
			 * @param string $html with replace target to author id block.
			 * @param bool $is_first_post is it a converting the content from the parent post to the first post.
			 */
			$ljsc->new_value( 'authors_id', apply_filters( 'lp_authors_html_id_wrap' , '<span class="lp-authorID">##id##</span>', false ) );
			/**
			 * Filter to allow you change the html for the author ID.
			 *
			 * @since 1.3.19
			 ...
			 * @param bool $is_first_post is it a converting the content from the parent post to the first post.
			 */
			$ljsc->new_value( 'authors_avatar', apply_filters( 'lp_authors_html_avatar_wrap' , '<span class="live-author-gravatar">##avatar##</span>', false ) );

			/**
			 * Filter to allow you change the html for the author Name.
			 *
			 * @since 1.3.19
			 *
			 * @param string $html with replace target to author Name block.
			 * @param bool $is_first_post is it a converting the content from the parent post to the first post.
			 */
			$ljsc->new_value( 'authors_name', apply_filters( 'lp_authors_html_name_wrap' , '<span class="live-author-name">##name##</span>', false ) );

			/**
			 * Filter to allow you change the html for the author html wrap.
			 *
			 * @since 1.3.19
			 *
			 * @param string $html with replace target to author html wrap.
			 * @param bool $is_first_post is it a converting the content from the parent post to the first post.
			 */
			$ljsc->new_value( 'author_wrap', apply_filters( 'lp_author_html_wrap' , '<span class="live-update-author live-update-author-##slugg##">##html##</span>', false ) );
			/**
			 * Filter to allow you change the html for the authors html wrap.
			 *
			 * @since 1.3.19
			 *
			 * @param string $html with replace target to authors html wrap.
			 * @param bool $is_first_post is it a converting the content from the parent post to the first post.
			 */
			$ljsc->new_value( 'authors_wrap', apply_filters( 'lp_authors_html_wrap' , '<div class="live-update-authors">##html##</div>', false ) );
			/**
			 * Filter to allow you change the html wrap arrond the content.
			 *
			 * @since  1.4.4
			 *
			 * @param string $html with replace target to content block.
			 * @param bool $is_first_post is it a converting the content from the parent post to the first post.
			 */
			$ljsc->new_value( 'content_wrap', apply_filters( 'livepress_content_wrap', '<div class="livepress-update-content-wrap">###content###</div>', false ) );
	...
	public function add_modified_tinymce( $plugin_array ) {
		if ( $this->livepress_config->debug() ) {
			...
			if ( $this->livepress_config->script_debug() ) {
				$plugin_array['livepress'] = LP_PLUGIN_URL . 'js/editor_plugin_release.full.js?v=' . LP_PLUGIN_SCRIPT_VERSION;
			} else {
				$plugin_array['livepress'] = LP_PLUGIN_URL . 'js/editor_plugin_release.min.js?v=' . LP_PLUGIN_SCRIPT_VERSION;
			}
		}
	...
	function wp_print_footer_scripts() {
		...
		$fb_app_id = '249643311490';
		if ( isset( $settings['facebook_app_id'] ) ) {
			$fb_app_id = $settings['facebook_app_id'];
		}
		$locale = get_locale();
		?><div id="fb-root" class="lp_facebook-jssdk"></div>
			<script>(function(d, s, id) { var js, fjs = d.getElementsByTagName(s)[0]; if (d.getElementById(id)) return; js = d.createElement(s); js.id = id; js.src = 'https://connect.facebook.net/<?php echo $locale; ?>/sdk.js#xfbml=1<?php echo $fb_app_id; ?>&version=v2.8'; fjs.parentNode.insertBefore(js, fjs); }(document, 'script', 'facebook-jssdk'));</script>
		<?php
	}
